...
impl ChessSettings {
    fn new(board: Board, turn: Turn) -> Self {
        Self {
            board: board,
            turn: turn,
        }
...
mod chess_ui {
    ...
    impl Application for ChessUI {
        ...
        fn new(flags: Self::Flags) -> (Self, iced::Command<Self::Message>) {
            (ChessUI::new(flags.board, flags.turn), iced::Command::none())
        }
        ...
        fn update(&mut self, message: Self::Message) -> iced::Command<Self::Message> {
            match message {
                CellClickedEvent::Clicked(pos) => {
                    ...
                    match self
                        .board
                        .move_piece(&mut self.turn, self.current_selected.unwrap(), pos)
                    {
                        Ok(_) => {
                            self.current_selected = None;
                            self.legal_moves.clear();
        ...
        fn view(&self) -> Element<'_, Self::Message> {
            ...
            for i in (0..8).rev() {
                ...
                for j in 0..8 {
                    ...
                    let cell = self.board.look_up_cell((i, j).into()).unwrap();
